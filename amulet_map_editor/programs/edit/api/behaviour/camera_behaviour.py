...
class CameraBehaviour(BaseBehaviour):
    ...
    def _on_input_press(self, evt: InputPressEvent):
        ...
        if evt.action_id == ACT_CHANGE_PROJECTION:
            ...
            if not hasattr(self.canvas, '_touch_controls_enabled') or not self.canvas._touch_controls_enabled:
                self.canvas.SetFocus()
    ...
    def _on_input_release(self, evt: InputReleaseEvent):
        ...
        if evt.action_id == ACT_CHANGE_MOUSE_MODE:
            ...
            if (self._camera_moved_with_touch and 
                hasattr(self.canvas, '_touch_controls_enabled') and 
                self.canvas._touch_controls_enabled and
                hasattr(self.canvas, 'set_mouse_selection_mode') and
                self._saved_mouse_selection_mode is not None):
    ...
    def _on_input_held(self, evt: InputHeldEvent):
        ...
        if (hasattr(self.canvas, '_touch_controls_enabled') and 
            self.canvas._touch_controls_enabled and
            hasattr(self.canvas, '_mouse_selection_mode') and
            any((forward, up, right))):
